...
CSceneGameplay::CSceneGameplay() : CScene()
{
  ...
  GameplayMenu = new hud::menu::CGameplayMenu(GlobalTranslations, Keyboard);
...
void CSceneGameplay::events()
{
  ...
  // Menu
  if (Keyboard->pressed(KEY_KEY_D, engine::EVENT_ONCE)) {
    GameplayMenu->toggle();
    Ayron->toggleControl();
...
void CSceneGameplay::manageCharacterJumps()
{
  if (Ayron->hasControl()) {
    ...
    // Keyboard
    if (Keyboard->pressed(KEY_KEY_V, engine::EVENT_ONCE)) {
...
{
  if (Keyboard->pressed(KEY_SPACE, engine::EVENT_ONCE)) {
